...
class saxonTheme_meta_box {
	...
	function js_css() {
		...
		$base_url = get_template_directory_uri(). '/functions/meta/';// get URL of the directory of current file, this works in both theme or plugin
	
		wp_enqueue_style('saxonTheme-meta-box', $base_url . '/meta-box.css');
		wp_enqueue_script('saxonTheme-meta-box', $base_url . '/meta-box.js', array('jquery'), null, true);
	}
	...
	function insert_images() {
		if (!isset($_POST['saxonTheme-insert']) || empty($_POST['attachments'])) return;
		...
		foreach ($_POST['attachments'] as $attachment_id => $attachment) {
			...
			$li .= "<a title='" . __('Delete this image') . "' class='saxonTheme-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>" . __('Delete') . "</a>";
	...
	function check_field_date() {
		if ($this->has_field('date') && $this->is_edit_page()) {
			...
			wp_enqueue_style('saxonTheme-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('saxonTheme-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
		}
	}
	...
	function check_field_time() {
		if ($this->has_field('time') && $this->is_edit_page()) {
			...
			wp_enqueue_style('saxonTheme-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('saxonTheme-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
			wp_enqueue_script('saxonTheme-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array('rw-jquery-ui'));
		}
	}
// Check field image
	...
	function check_field_slider() {
		...
		// add style and script, use proper jQuery UI version
			wp_enqueue_script('saxonTheme-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('saxonTheme-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'saxonTheme_meta_box_nonce');
	...
	function show_field_begin($field, $meta) {
		echo "<th class='saxonTheme-label'><label for='{$field['id']}'>{$field['name']}</label></th><td class='rw-field'>";
	}
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='saxonTheme-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_textarea($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='saxonTheme-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_select($field, $meta) {
		...
		$this->show_field_begin($field, $meta);
		echo "<select class='saxonTheme-select' name='{$field['id']}" . ($field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'") . ">";
...
function show_field_plaintext($field, $meta) {
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			...
			echo '<ol class="saxonTheme-upload">';
			foreach ($meta as $att) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='saxonTheme-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete') . "</a>)</li>";
			}
			echo '</ol>';
		}
	...
	function show_field_image($field, $meta) {
		...
		echo "<input type='hidden' class='saxonTheme-images-data' value='{$post->ID}|{$field['id']}|$nonce_sort' />
			  <ul class='saxonTheme-images rw-upload' id='saxonTheme-images-{$field['id']}'>";
		...
		// re-arrange images with 'menu_order', thanks Onur
			...
			foreach ($images as $image) {
				...
				echo "<li id='item_$image'>
						<img src='$src' />
						<a title='" . __('Delete this image') . "' class='saxonTheme-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete') . "</a>
		...
		echo "<a href='#' class='saxonTheme-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images') . "</a>";
	}

	function show_field_color($field, $meta) {
		if (empty($meta)) $meta = '#';
		$this->show_field_begin($field, $meta);
		echo "<input class='saxonTheme-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='saxonThemel-color-select' rel='{$field['id']}'>" . __('Select a color') . "</a>
			  <a href='#' class='saxonTheme-color-select' style='display: none;'>" . __('Hide Picker') . "</a>
			  <div style='display:none; position: absolute; background: #f0f0f0; border: 1px solid #ccc; z-index: 100;' class='BusinessCasual-color-picker' rel='{$field['id']}'></div>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox_list($field, $meta) {
		...
		$html = array();
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='saxonTheme-checkbox_list' name='{$field['id']}[]' value='$key'" . checked(in_array($key, $meta), true, false) . " /> $value";
		}
	...
	function show_field_date($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='saxonTheme-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_time($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='saxonTheme-time' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}
function show_field_single_image($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='saxonTheme-single-image' name='{$field['id']}' id='{$field['id']}' value='$meta' size='18' />";
		echo "<input type='button' class='saxonTheme-upload-single-image button-secondary' value='Choose Image'/><br />";
		$this->show_field_end($field, $meta);
	}
	
	function show_field_slider($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<div class='saxonTheme-slider' rel='{$field['id']}'></div>";		
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'saxonTheme_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('saxonTheme_meta_box_Validate') && method_exists('saxonTheme_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('saxonTheme_meta_box_Validate', $field['validate_func']), $new);
			}
